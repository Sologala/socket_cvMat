...
class msgdeWraper {
   public:
    cv::Mat operator()(proto_cv::msgMat &msg) {
        ...
        std::copy(reinterpret_cast<unsigned char *>(const_cast<char *>(msg.mat_data().data())),
                  reinterpret_cast<unsigned char *>(const_cast<char *>(msg.mat_data().data()) + dataSize), img.data);
        return img.clone();
    }
};
